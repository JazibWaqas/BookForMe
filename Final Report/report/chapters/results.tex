...
\noindent Focal loss~\cite{lin2017} was applied to address the class imbalance, down-weighting easy majority examples and focusing training on the harder minority class boundaries.
...
\toprule
\textbf{Model} & \textbf{Notes} & {\scriptsize\textbf{Accuracy}} & \textbf{Macro-F1} & \textbf{Wtd.\ F1} \\
\midrule
bert-base-uncased
...
\textbf{distilbert-base-uncased}
  & \textbf{Best overall; fastest inference (47 ms)}
...
\begin{enumerate}
  \item \textbf{Best accuracy} (90.5\%) and \textbf{macro-F1} (91.7\%) across all
        six models tested.
  \item \textbf{Fastest inference}: 47 ms per query on CPU, versus
        120 ms for bert-base-multilingual-cased.  This is critical given
        the 60-second total WhatsApp response budget (NFR-PERF).
...
\begin{figure}[h!]
\centering
\includegraphics[width=1\textwidth]{distilbert_per_class_prf.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width=1\textwidth]{distilbert_confusion_matrix.png}
...
\toprule
\textbf{Test ID} & \textbf{Scenario} & \textbf{Expected / Actual Behaviour} & {\scriptsize\textbf{Result}} \\
\midrule
TC-01
...
\begin{enumerate}
  \item \textbf{Compact models outperform large multilingual models on small
        ...
        fine-tuning on 694 utterances contributes more than additional model capacity.
...
\begin{itemize}
  ...
  \item \textbf{Latency on free-tier deployment:} Response times of 38 to 52 seconds,
